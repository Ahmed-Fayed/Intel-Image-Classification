    ...
    for epoch in tqdm(range(Epochs)):
        ...
        with tf.device(device_name=device):
            for (x, y) in val_dataset:
                ...
                y_pred = model(x)
                val_loss = val_loss(y, y_pred)
                val_accuracy(y, y_pred)
        
        tf.print('epoch {}: train loss {} ; val_loss {} ; train accuracy {} ; val_accuracy {}'.format(epoch, loss, val_loss, train_accuracy.result(), val_accuracy.result()))
        
        DetectOverFittingCallback(1.3, train_accuracy.result(), val_accuracy.result())
